...
const REACT_APP_GOOGLE_SCRIPT_URL =
...
function fetchWithJsonp(userId: string): Promise<any> {
  return new Promise((resolve, reject) => {
    const callbackName =
      "googleSheetsCallback_" + Math.random().toString(36).substr(2, 9);
...
function saveWithForm(data: any): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    const formId =
      "googleSheetsForm_" + Math.random().toString(36).substr(2, 9);
    const iframeId =
      "googleSheetsIframe_" + Math.random().toString(36).substr(2, 9);
    ...
    iframe.onload = () => {
      try {
        ...
        const responseText =
          iframeDocument?.body.innerText || iframeDocument?.body.textContent;
