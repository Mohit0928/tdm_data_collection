...
export default function FormEditor({ config, onChange }: FormEditorProps) {
  ...
  const handleFieldChange = (e: React.ChangeEvent<HTMLInputElement | { name?: string; value: unknown }> | SelectChangeEvent) => {
  ...
  const handleAddCodedOption = () => {
    if (newLabel.trim()) {
      const updatedOptions = [...codedOptions, { code: newCode, label: newLabel }];
  ...
  const handleDeleteCodedOption = (codeToDelete: number) => {
    const updatedOptions = codedOptions.filter(option => option.code !== codeToDelete);
    setCodedOptions(updatedOptions);
    setNewField(prev => ({
      ...prev,
      codedOptions: updatedOptions
  ...
  return (
    <Box sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth margin="normal">
                ...
                >
                  ...
                  <MenuItem value="codedValue">Coded Value (Multiple Options)</MenuItem>
                </Select>
              </FormControl>
            </Grid>
            <Grid item xs={12} sm={6}>
              ...
              <>
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) => {
                      setNewField(prev => {
                        const currentLabels = prev.booleanLabels || { trueLabel: '1 - Yes', falseLabel: '0 - No' };
                        return {
                          ...prev,
                          booleanLabels: {
                            ...currentLabels,
            ...
            {newField.type === 'codedValue' && (
              <Grid item xs={12}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  <Typography variant="body2" sx={{ mr: 2 }}>Coded Options:</Typography>
                  <Button 
                    variant="outlined" 
                    size="small" 
                    startIcon={<EditIcon />}
        ...
        {/* Current Fields */}
        <Box>
          <Typography variant="subtitle1" gutterBottom>Current Fields</Typography>
          {config.groups.map((group) => (
      ...
      <Dialog open={openCodedDialog} onClose={handleCloseCodedDialog} maxWidth="sm" fullWidth>
          ...
          <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
            {codedOptions.map((option) => (
              <ListItem
                key={option.code}
                secondaryAction={
                  <IconButton edge="end" onClick={() => handleDeleteCodedOption(option.code)}>
                    <DeleteIcon />
                  </IconButton>
                }
              >
